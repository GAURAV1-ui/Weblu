  ...
  return (
    <>
      <header className="hidden md:flex items-center justify-between w-full px-8 mt-4">
        ...
        <motion.div
          className="flex p-1 backdrop-blur-md sm:rounded-full text-gray-900 dark:text-white"
          initial={{ y: -100, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
        >
          <ul className="flex flex-wrap items-center justify-center gap-y-1 text-[1rem] font-inter font-bold text-black dark:text-white">
            {links.map((link) => (
              <motion.li
